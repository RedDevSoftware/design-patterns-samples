...
public class StructureHouseImpl implements IStructureFactory {

    private String[] materials = {"Material PL","Material PY","Material PK"};

    @Override
    public void getMaterials() {
        for (String material : materials) {
            GenericUtils.infoLogger(String.format("Need %s of %s to start a %s.",
                    (int) (Math.random()*100), material, Constans.HOUSE));
        }
    ...
    public void stopBuilding() {
        GenericUtils.infoLogger("Init stopBuilding in StructureHouseImpl");
    }
}
